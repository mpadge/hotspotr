...
#' structured field generated by \code{ives}
...
#' be generated with specified values of \code{alpha} and \code{size=10}
#' @param seed Random seed
#'
#' @return Nothing (dumps statistics to screen)
#'
#' @seealso \code{test_hotspots}
#'
#' @examples
#' \dontrun{
...
#'
#' @export
run_tests <- function (nbs, alpha=c(0.1, 0.1), nt=100, ntests=100, 
                       neutral=FALSE, ac_type='moran', z, seed) 
{
    ...
    if (missing (z))
    {
        dat <- ives (size=10, nt=nt, sd0=0.1, alpha=alpha)
        if (missing (nbs)) nbs <- dat$nbs
        if (neutral)
            z <- rcpp_neutral_hotspots (nbs=nbs, alpha_t=alpha [1],
        ...
        else
        {
            if (missing (seed))
                ...
                z <- ives (size=10, nt=nt, sd0=0.1, alpha=alpha, seed=seed)$z
        }
    }
    ...
    for (i in 1:2)
    {
        t2 <- test_hotspots (z, nbs=nbs, alpha=c(alpha_t, alpha_s [i]), ntests=ntests)

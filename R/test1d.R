#' test1d
#'
#' Tests an observed matrix of data (\code{ymat}) against a one-dimensional
#' neutral model
#'
...
#' result <- test1d (ymat, alpha, ntests=10)
#' }
#'
#' @export
test1d <- function (ymat, alpha=c(0.1, 0.1), ntests=100, actype='moran',
                    plot=FALSE)
{
    if (!is.matrix (ymat)) 
    ...
    for (i in 1:ntests)
    {
        yi <- neutral1d (size, alpha=a0, nt=nt0)
